...
export function loadConfig(): DevDashConfig {
  ...
  cached = {
    ...
    groups: mergeGroups(user.groups ?? [], utils.map(u => u.name), tests.map(t => t.name)),
...
/** Insert user groups between the built-in 'infra'/'utils'/'tests' groups and the 'workers'/'other' fallback groups. */
function mergeGroups(userGroups: GroupDef[], utilsNames: string[], testsNames: string[]): GroupDef[] {
  const infra = BUILTIN_GROUPS.find(g => g.id === 'infra')!
  const utils = { ...BUILTIN_GROUPS.find(g => g.id === 'utils')!, match: { in: utilsNames } }
  const tests = { ...BUILTIN_GROUPS.find(g => g.id === 'tests')!, match: { in: testsNames } }
  const workers = BUILTIN_GROUPS.find(g => g.id === 'workers')!
  const other = BUILTIN_GROUPS.find(g => g.id === 'other')!
  const filtered = userGroups.filter(g => !['infra', 'utils', 'tests', 'workers', 'other'].includes(g.id))
  return [infra, utils, tests, ...filtered, workers, other]
}
