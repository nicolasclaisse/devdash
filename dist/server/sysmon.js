...
export function getSysProcesses() {
...
function categorize(cmd) {
    if (cmd.match(/Slack/))
        return 'Slack';
    if (cmd.match(/Arc.*Renderer|arc.*renderer/))
        return 'Arc (renderers)';
    if (cmd.match(/Arc/))
        return 'Arc';
    if (cmd.match(/Brave.*Renderer|Brave.*Helper \(Renderer\)/))
        return 'Brave (renderers)';
    if (cmd.match(/Brave/))
        return 'Brave';
    if (cmd.match(/Google Chrome.*Renderer/))
        return 'Chrome (renderers)';
    if (cmd.match(/Google Chrome/))
        return 'Chrome';
    if (cmd.match(/tsserver/))
        return 'VSCode tsserver';
    if (cmd.match(/eslintServer/))
        return 'VSCode ESLint';
    if (cmd.match(/Code Helper \(Plugin\)/))
        return 'VSCode (plugins)';
    if (cmd.match(/Code Helper \(Renderer\)/))
        return 'VSCode (renderers)';
    if (cmd.match(/Visual Studio Code|Code\.app/))
        return 'VSCode';
    if (cmd.match(/node.*devdash/))
        return 'devdash';
    if (cmd.match(/next-server|next start/))
        return 'Next.js';
    if (cmd.match(/nest/))
        return 'NestJS';
    if (cmd.match(/[Nn]ode/))
        return 'Node.js (other)';
    if (cmd.match(/postgres/))
        return 'PostgreSQL';
    if (cmd.match(/redis/))
        return 'Redis';
    if (cmd.match(/mds_stores/))
        return 'Spotlight';
    if (cmd.match(/WebKit|WebContent/))
        return 'WebKit';
    if (cmd.match(/claude/))
        return 'Claude Code';
    if (cmd.match(/Dock|Finder|WindowServer|loginwindow|SystemUIServer/))
        return 'macOS UI';
    if (cmd.match(/Xcode/))
        return 'Xcode';
    if (cmd.match(/simulator/i))
        return 'Simulator';
...
export function getMemBreakdown() {
    try {
        ...
        return { active: 0, wired: 0, compressed: 0, fileCache: 0, free: 0 };
    }
}
