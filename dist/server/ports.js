...
function getActivePorts() {
    ...
    try {
        ...
        for (const line of out.split('\n').slice(1)) {
            ...
            if (!map.has(port))
                map.set(port, { pid: Number(pid), command });
        }
    }
    catch { /* ignore */ }
    return map;
}
function getMemForPids(pids) {
    const map = new Map();
    if (!pids.length)
        return map;
    try {
...
export function killPort(port) {
    try {
        const pids = execSync(`lsof -ti TCP:${port} 2>/dev/null`, { stdio: ['ignore', 'pipe', 'pipe'] }).toString().trim();
        if (!pids)
            return false;
