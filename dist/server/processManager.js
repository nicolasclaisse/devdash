...
export class ProcessManager {
    ...
    async waitFor(name, condition, timeoutMs = 120_000) {
        ...
        while (Date.now() - start < timeoutMs) {
            ...
            if (condition === 'process_completed_successfully') {
                ...
                if (['running', 'healthy', 'completed'].includes(status))
                    return true;
                if (status === 'failed')
                    return false;
            }
                ...
                if (status === 'running' && state?.def) {
                    if (await this.runHealthCheck(state.def))
                        return true;
                }
            }
            await delay(1000);
    ...
    pollHealthy(name) {
        ...
        const check = async () => {
            ...
            if (attempts < maxAttempts)
                ...
                log(`[devdash] ${name} health check failed after ${maxAttempts} attempts`);
        };
        setTimeout(check, period);
    }
    async startOne(name) {
        ...
        if (!this.autostartNames.has(name)) {
            for (const serviceName of this.autostartNames) {
                ...
                if (!ok) {
                    log(`[devdash] ${name} autostart ${serviceName} not satisfied — aborting`);
                    s.status = 'failed';
                    return;
                }
            }
        ...
        for (const [dep, condition] of Object.entries(s.def.depends_on)) {
            ...
            if (depStatus === 'stopped' || depStatus === 'failed') {
                this.startOne(dep).catch((e) => log(`[devdash] ${dep} error: ${e.message}`));
            }
            log(`[devdash] ${name} waiting for ${dep} (${condition})`);
            const ok = await this.waitFor(dep, condition);
            if (!ok) {
                log(`[devdash] ${name} dep ${dep} not satisfied — aborting`);
                s.status = 'failed';
                return;
            }
        }
    ...
    startMany(names) {
        for (const name of names) {
            if (this.states.has(name)) {
                this.startOne(name).catch((e) => log(`[devdash] ${name} error: ${e.message}`));
            }
        }
    }
    startAll() {
    ...
    sampleStats() {
        const map = new Map();
        const roots = [...this.states.values()].map(s => s.pid).filter(Boolean);
        if (!roots.length)
            return map;
        try {
            // Collect all processes: pid → { ppid, cpu, mem }
            const out = execSync('ps -A -o pid=,ppid=,pcpu=,rss=', { stdio: ['ignore', 'pipe', 'pipe'], timeout: 2000 }).toString();
            const all = new Map();
